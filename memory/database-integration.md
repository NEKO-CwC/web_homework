...
- `pnpm prisma:smoke` passed against the hosted database. The script creates a temporary customer, runs checkout, virtual payment, merchant shipment, receipt confirmation, after-sale creation, merchant after-sale handling, and review through `PrismaMallWriteService`, then cleans temporary records and restores product stock.
